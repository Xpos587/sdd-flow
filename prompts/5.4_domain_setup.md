...
- Graph ontology from `docs/common/graph-schema.md`
...
- Set workflow context for AI agents (epic, step, domain, dev stage)
...
- REMOVED: `[communication]` section - no cross-domain restrictions needed
...
Allowed folders: src/backend/domains/{DOMAIN_NAME}/, src/frontend/domains/{DOMAIN_NAME}/, src/shared/dto/{DOMAIN_NAME}/, docs/domains/{DOMAIN_NAME}/
...
- Workflow context provided in structured format
...
1. **Domain Isolation Check:**
   ...
   - Verify Context.toml has only 2 sections: [domain] and [workflow]
   - Ensure no deprecated sections are present ([communication] removed)
   - Check that agent prompt contains correct domain rules
   - Validate that domain boundaries are clearly defined

2. **Component Validation:**

   - Verify all GRAPH*TAG IDs follow the format: `{LAYER}{DOMAIN}*{COMPONENT}\_{Name}`
...
4. **Workflow Context Validation:**

   - Verify workflow section contains all required fields
   - Check current_epic, current_step, current_domain, current_dev_stage are populated
   - Ensure workflow context matches current project state
...
5. **Context.toml Validation:**
   ...
   - Confirm [communication] section has been removed
...
4. **Generate simplified Context.toml** - Create MAXIMALLY simplified configuration with workflow context
